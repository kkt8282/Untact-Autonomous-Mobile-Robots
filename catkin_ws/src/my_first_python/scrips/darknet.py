...
def print_detections(detections, coordinates=False):
    ...
    for label, confidence, bbox in detections:
        if label == 'person' or label == 'mask' or label == 'no_mask':
            x, y, w, h = bbox
            if coordinates:
...
def draw_boxes(detections, image, colors):
    import cv2
    for label, confidence, bbox in detections:
        if (label == 'person' or label== 'no_mask' or label == 'mask'):
...
def remove_negatives(detections, class_names, num):
    ...
    for j in range(num):
        for idx, name in enumerate(class_names):
            if detections[j].prob[idx] > 0.45 and (name == 'person' or name== 'no_mask' or name == 'mask'):
